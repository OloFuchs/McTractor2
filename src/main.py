...
class McTractor(App):
        ...
        def on_touch_down(window: Window, touch: MouseMotionEvent) -> bool:
            ...
            if self.root.ids["rgb"].collide_point(*touch.pos):
                ...
                if self.root.ids["mainTab"].current_tab.text == "Crop":
                    ...
                    posTmp = (
                        (float(touch.pos[0]) - x0) / minX,
                        (float(touch.pos[1]) - y0) / minY,
                    )
                    if self.touchCnt == 0:
    ...
    def mainTabImagingFunction(self, rgb):
        ...
        start_point = (
            int(self.firstPos[0] * rgb.shape[1]),
            int(self.firstPos[1] * rgb.shape[0]),
        )
        end_point = (
            int(self.secondPos[0] * rgb.shape[1]),
        ...
        if self.view_name == "rgb":
            ...
            texture.blit_buffer(
                data, bufferfmt="ubyte", colorfmt="bgr", mipmap_generation=False
            )
            self.root.ids[self.view_name].texture = texture

        if (
            (self.view_name == "filter")
            or (self.view_name == "features")
            or (self.view_name == "control")
        ):
            if self.touchCnt == 2:
                x1 = start_point[0]
                y1 = start_point[1]
                ...
                low_hsv = np.array(
                    [
                        self.root.ids.slider_LR.value,
                        self.root.ids.slider_LG.value,
                        self.root.ids.slider_LB.value,
                    ]
                )
                high_hsv = np.array(
                    [
                        self.root.ids.slider_HR.value,
                        self.root.ids.slider_HG.value,
                        self.root.ids.slider_HB.value,
                    ]
                )
                ...
                if self.view_name == "filter":
                    ...
                    croptexture = Texture.create(
                        size=(croppedRGB.shape[1], croppedRGB.shape[0]), icolorfmt="bgr"
                    )
                    croptexture.blit_buffer(
                        cropdata,
                        bufferfmt="ubyte",
                        colorfmt="bgr",
                    ...
                    difftexture = Texture.create(
                        size=(diffused.shape[1], diffused.shape[0]), icolorfmt="bgr"
                    )
                    difftexture.blit_buffer(
                        diffdata,
                        bufferfmt="ubyte",
                        colorfmt="bgr",
                        mipmap_generation=False,
                    ...
                    (self.view_name == "features")
                    or (self.view_name == "control")
                    or (self.view_name == "graphrgb")
                ):
                    ...
                    ret, bwImg = cv2.threshold(
                        grayImg, grayTreshold, 255, cv2.THRESH_BINARY
                    )
                    ...
                    lowRadius = self.root.ids.sliderRmin.value * max(
                        imgHeight, imgWidth
                    )
                    highRadius = self.root.ids.sliderRmax.value * max(
                        imgHeight, imgWidth
                    )
                    cntCircle = 0
                    if len(contours) > 0:
                        ...
                        for i in range(len(contours)):
                            ...
                            if (radius > lowRadius) and (radius < highRadius):
                                ...
                                center = (int(x), int(y))
                                radius = int(radius)
                                bwImgWithBlobsRGB = cv2.circle(
                                    bwImgWithBlobsRGB, center, radius, (0, 0, 255), 3
                                )
                    if cntCircle > 0:
                        sumCircleRadius = 0.0
                        sumWeightCircle = 0.0
                        for i in range(cntCircle):
                            ...
                            sumWeightCircle = (
                                sumWeightCircle + radius * radius * x * 1.0
                            )
                        M = sumWeightCircle / sumCircleRadius
                        intM = int(M)
                        bwImgWithBlobsRGB = cv2.line(
                            bwImgWithBlobsRGB,
                            (intM, 0),
                            (intM, imgHeight),
                            (0, 0, 255),
                    ...
                    if self.view_name == "features":
                        ...
                        grayTexture = Texture.create(
                            size=(grayImg.shape[1], grayImg.shape[0]), icolorfmt="bgr"
                        )
                        grayTexture.blit_buffer(
                            grayData,
                            bufferfmt="ubyte",
                            colorfmt="luminance",
                            mipmap_generation=False,
                        ...
                        bwTexture = Texture.create(
                            size=(
                                bwImgWithBlobsRGB.shape[1],
                                bwImgWithBlobsRGB.shape[0],
                            ),
                            icolorfmt="bgr",
                        )
                        bwTexture.blit_buffer(
                            bwData,
                            bufferfmt="ubyte",
                            colorfmt="bgr",
                            mipmap_generation=False,
                    ...
                    elif (self.view_name == "control") or (
                        self.view_name == "graphrgb"
                    ):
                        # print("control")
                        intrefLine = int(
                            imgWidth * self.root.ids.refLineSlider.value / 100.0
                        )
                        ...
                        # and we can use Kp range [0, 2] for initial testing
                        ...
                        self.root.ids.errorLabel.text = "Error: {0:.3f}".format(
                            self.err / 10000
                        )
                        self.root.ids.errorLabel.texture_update()
                        self.root.ids.controlLabel.text = "Control: {0:.3f}".format(
                            ctrlU
                        )
